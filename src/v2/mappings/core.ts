...
import { findEthPerToken, getNativeTokenPriceInUSD, getTrackedLiquidityUSD, getTrackedVolumeUSD, calculateAPY } from '../../common/pricing'
...
export function handleSync(event: Sync): void {
  ...
  log.info("Reserve Native Token: {}", [pair.reserveNativeToken.toString()]);
  log.info("Native Token Price: {}", [bundle.nativeTokenPrice.toString()]);
...
export function handleMint(event: Mint): void {
  ...
  let bundle = Bundle.load('1')!
  let amountTotalUSD = token1.derivedNativeToken
    .times(token1Amount)
    .plus(token0.derivedNativeToken.times(token0Amount))
...
export function handleSwap(event: Swap): void {
  ...
  let derivedAmountETH = token1.derivedNativeToken
    .times(amount1Total)
    .plus(token0.derivedNativeToken.times(amount0Total))
    .div(BigDecimal.fromString('2'))
  let derivedAmountUSD = derivedAmountETH.times(bundle.nativeTokenPrice)
  ...
  if (bundle.nativeTokenPrice.equals(ZERO_BD)) {
    ...
    trackedAmountETH = trackedAmountUSD.div(bundle.nativeTokenPrice)
  }
  ...
  uniswap.totalVolumeNativeToken = uniswap.totalVolumeNativeToken.plus(trackedAmountETH)
  ...
  uniswapDayData.dailyVolumeNativeToken = uniswapDayData.dailyVolumeNativeToken.plus(trackedAmountETH)
  ...
  pairDayData.APY = calculateAPY(pairDayData.dailyVolumeUSD, pairDayData.reserveUSD)
  pairDayData.save()
